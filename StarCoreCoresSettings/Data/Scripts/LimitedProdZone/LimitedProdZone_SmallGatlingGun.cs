                ...
                if (isServer)
                {
                    ...
                    {                        
                        ...
                        if (Vector3D.Distance(weapon.GetPosition(), beacon.GetPosition()) < 20000)
                        {
                            ...
                            bool isBasicSmallGatlingGun = false;
                            isBasicSmallGatlingGun = strSubBlockType.Contains("Basic");

                            if (isBasicSmallGatlingGun == false)
                            {
								inZone = true;
        ...
        private void WorkingStateChange(IMyCubeBlock block)
        {
            if (!weapon.Enabled)
            {
                foreach (var beacon in beaconList)
                {
                    ...
                    {
                        string strSubBlockType = weapon.BlockDefinition.SubtypeId.ToString();
                        Boolean isBasicSmallGatlingGun = false;
                        isBasicSmallGatlingGun = strSubBlockType.Contains("Basic");

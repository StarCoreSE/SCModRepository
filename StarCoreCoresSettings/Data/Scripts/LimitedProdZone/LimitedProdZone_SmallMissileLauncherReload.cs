                ...
                if (isServer)
                {
                    ...
                    {                        
                        ...
                        if (Vector3D.Distance(weapon.GetPosition(), beacon.GetPosition()) < 20000)
                        {
                            ...
                            bool isBasicSmallMissileLauncherReload = false;
                            isBasicSmallMissileLauncherReload = strSubBlockType.Contains("Basic");

                            if (isBasicSmallMissileLauncherReload == false)
                            {
								inZone = true;
        ...
        private void WorkingStateChange(IMyCubeBlock block)
        {
            if (!weapon.Enabled)
            {
                foreach (var beacon in beaconList)
                {
                    ...
                    {
                        string strSubBlockType = weapon.BlockDefinition.SubtypeId.ToString();
                        Boolean isBasicSmallMissileLauncherReload = false;
                        isBasicSmallMissileLauncherReload = strSubBlockType.Contains("Basic");

            ...
            try
            {
                if (PointCheckHelpers.timer % 60 == 0 && broadcaststat)
                {
                    bool tick100 = _count % 100 == 0; _count++; if (_count - _fastStart < 300 || tick100)
                    {
                        ...
                        {
                            var grid = entity as MyCubeGrid; if (grid != null && GridExtensions.HasBlockWithSubtypeId(grid, "LargeFlightMovement") || GridExtensions.HasBlockWithSubtypeId(grid, "RivalAIRemoteControl"))
                            {

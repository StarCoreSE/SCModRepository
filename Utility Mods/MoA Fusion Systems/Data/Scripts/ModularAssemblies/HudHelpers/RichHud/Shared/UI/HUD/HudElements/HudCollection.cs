...
namespace RichHudFramework
{
    namespace UI
    {
        ...
        /// </summary>
        public class HudCollection<TElementContainer, TElement> : HudElementBase,
            ...
            where TElement : HudNodeBase
        {
            ...
            public TElementContainer this[int index]
            {
                get
                {
                    if (hudCollectionList.Count == 0 || index < 0 || index >= hudCollectionList.Count)
                        throw new Exception(
                            $"Collection index out of range. Index: {index} Count: {hudCollectionList.Count}");
            ...
            {
                return hudCollectionList.GetEnumerator();
            }
            ...
            public bool Remove(TElementContainer entry)
            {
                if (entry.Element.Parent == this && hudCollectionList.Count > 0)
                    ...
                    {
                        var success = entry.Element.Unregister();
            ...
            public bool Remove(Func<TElementContainer, bool> predicate)
            {
                ...
                {
                    var index = hudCollectionList.FindIndex(x => predicate(x));
            ...
            public bool RemoveAt(int index)
            {
                if (hudCollectionList[index].Element.Parent == this && hudCollectionList.Count > 0)
                {
                    ...
                    var success = element.Unregister();
            ...
            public void Clear()
            {
                NodeUtils.UnregisterNodes<TElementContainer, TElement>(this, children, hudCollectionList, 0,
                    hudCollectionList.Count);
            ...
            {
                return hudCollectionList.Contains(item);
            }
            ...
            public override bool RemoveChild(HudNodeBase child)
            {
                ...
                if (child.Parent == null && children.Remove(child))
                {

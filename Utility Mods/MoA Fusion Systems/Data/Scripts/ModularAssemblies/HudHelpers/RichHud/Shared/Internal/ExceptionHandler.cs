...
namespace RichHudFramework.Internal
{
    ...
    public sealed class ExceptionHandler : MySessionComponentBase
    {
        ...
        public override void LoadData()
        {
            ...
            IsServer = MyAPIGateway.Session.OnlineMode == MyOnlineModeEnum.OFFLINE ||
                       MyAPIGateway.Multiplayer.IsServer || IsDedicated;
            IsClient = !IsDedicated;

            WriteToLogAndConsole(
                $"Exception Handler Init. Dedicated: {IsDedicated}, IsServer: {IsServer}, IsClient: {IsClient}", true);
        }
        ...
        private void HandleExceptions()
        {
            if (exceptionCount > 0)
            {
                var exceptionText = GetExceptionText();
                exceptionCount = 0;
        ...
        private string GetExceptionText()
        {
            ...
            if (exceptionCount > exceptionLoopCount && errorTimer.ElapsedMilliseconds < exceptionLoopTime)
                errorMessage.AppendLine(
                    $"[Exception Loop Detected] {exceptionCount} exceptions were reported within a span of {errorTimer.ElapsedMilliseconds}ms.");

            for (var n = 0; n < exceptionMessages.Count - 1; n++)
        ...
        /// </summary>
        public static void ShowMissionScreen(string subHeading = null, string message = null,
            Action<ResultEnum> callback = null, string okButtonCaption = null)
        {
            Action messageAction = () =>
                MyAPIGateway.Utilities.ShowMissionScreen(ModName, subHeading, null, message, callback, okButtonCaption);
        ...
        private void FinishReload()
        {
            if (Reloading)
            {
                ...
                {
                    var success = true;
                    var typeName = clients[n].GetType().Name;

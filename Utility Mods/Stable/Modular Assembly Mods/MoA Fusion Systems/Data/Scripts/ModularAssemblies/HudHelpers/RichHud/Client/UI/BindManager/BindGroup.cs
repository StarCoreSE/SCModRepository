    ...
    namespace UI.Client
    {
        public sealed partial class BindManager
        {
            ...
            private partial class BindGroup : ReadOnlyApiCollection<IBind>, IBindGroup
            {
                ...
                /// </summary>
                public string Name =>
                    _instance.GetOrSetGroupMemberFunc(Index, null, (int)BindGroupAccessors.Name) as string;
                ...
                {
                    return (bool)_instance.GetOrSetGroupMemberFunc(Index, name, (int)BindGroupAccessors.DoesBindExist);
                }
                ...
                public bool DoesComboConflict(IReadOnlyList<IControl> newCombo, IBind exception = null)
                {
                    ...
                    for (var n = 0; n < newCombo.Count; n++)
                        indices[n] = newCombo[n].Index;

                    var data = new MyTuple<IReadOnlyList<int>, int>(indices, exception?.Index ?? -1);
                    return (bool)_instance.GetOrSetGroupMemberFunc(Index, data,
                        (int)BindGroupAccessors.DoesComboConflict);
                }
                ...
                /// <summary>
                ///     Replaces current bind combos with combos based on the given <see cref="BindDefinition" />[]. Does not register new
                ///     binds.
                /// </summary>
                public bool TryLoadBindData(IReadOnlyList<BindDefinitionData> bindData)
                {
                    return (bool)_instance.GetOrSetGroupMemberFunc(Index, bindData,
                        (int)BindGroupAccessors.TryLoadBindData);
                }
                ...
                public bool TryLoadBindData(IReadOnlyList<BindDefinition> bindData)
                {
                    ...
                    for (var n = 0; n < bindData.Count; n++)
                        defData[n] = new BindDefinitionData(bindData[n].name, bindData[n].controlNames);

                    return (bool)_instance.GetOrSetGroupMemberFunc(Index, defData,
                        (int)BindGroupAccessors.TryLoadBindData);
                }
                ...
                {
                    var index = (Vector2I)_instance.GetOrSetGroupMemberFunc(Index, name,
                        (int)BindGroupAccessors.GetBindFromName);
                ...
                {
                    var index = (Vector2I)_instance.GetOrSetGroupMemberFunc(Index,
                        new MyTuple<string, IReadOnlyList<int>>(bindName, combo),
                        (int)BindGroupAccessors.AddBindWithIndices);
                    return this[index.Y];
                }
                ...
                {
                    var index = (Vector2I)_instance.GetOrSetGroupMemberFunc(Index,
                        new MyTuple<string, IReadOnlyList<string>>(bindName, combo),
                        (int)BindGroupAccessors.AddBindWithNames);
                    return this[index.Y];
                }
                ...
                public IBind AddBind(string bindName, IReadOnlyList<ControlData> combo = null)
                {
                    ...
                    for (var n = 0; n < combo.Count; n++)
                        indices[n] = combo[n].index;

                    var index = (Vector2I)_instance.GetOrSetGroupMemberFunc(Index,
                        new MyTuple<string, IReadOnlyList<int>>(bindName, indices),
                        (int)BindGroupAccessors.AddBindWithIndices);
                    return this[index.Y];
                }
                ...
                {
                    var index = (int)_instance.GetOrSetGroupMemberFunc(Index, bindName,
                        (int)BindGroupAccessors.TryRegisterBindName);
                ...
                {
                    var index = (int)_instance.GetOrSetGroupMemberFunc(Index,
                        new MyTuple<string, IReadOnlyList<int>>(bindName, combo),
                        (int)BindGroupAccessors.TryRegisterBindWithIndices);
                ...
                public bool TryRegisterBind(string bindName, out IBind bind, IReadOnlyList<string> combo)
                {
                    var index = (int)_instance.GetOrSetGroupMemberFunc(Index,
                        new MyTuple<string, IReadOnlyList<string>>(bindName, combo),
                        (int)BindGroupAccessors.TryRegisterBindWithNames);
                ...
                {
                    var bindData =
                        _instance.GetOrSetGroupMemberFunc(Index, null, (int)BindGroupAccessors.GetBindData) as
                            BindDefinitionData[];
                    var definitions = new BindDefinition[bindData.Length];

                    for (var n = 0; n < bindData.Length; n++)
                ...
                {
                    return _instance.GetOrSetGroupMemberFunc(Index, null, (int)BindGroupAccessors.GetBindData) as
                        BindDefinitionData[];
                }

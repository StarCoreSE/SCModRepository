...
namespace RichHudFramework.UI
{
    ...
    public class TextBox : Label, IClickableElement
    {
        ...
        {
            get { return caret.ShowCaret; }
            set { caret.ShowCaret = value; }
        }
        ...
        {
            var useInput = allowInput || (MouseInput.HasFocus && HudMain.InputMode == HudInputMode.Full);
            ...
            if (useInput && EnableEditing)
            {
                ...
                {
                    var text = TextBoard.GetTextRange(selectionBox.Start, selectionBox.End);
                    DeleteSelection();
            ...
            {
                var isCursorHighlighting = false;
                ...
                if (UseCursor)
                {
                    ...
                    else if (!canHighlight && MouseInput.IsLeftClicked &&
                             (cursorPos - cursorStart).LengthSquared() > 16f)
                    {
                        canHighlight = true;
                ...
                if (SharedBinds.SelectAll.IsNewPressed)
                    selectionBox.SetSelection(Vector2I.Zero,
                        new Vector2I(TextBoard.Count - 1, TextBoard[TextBoard.Count - 1].Count - 1));
        ...
        private static int GetRichTextMinLength(RichText text)
        {
            ...
            for (var n = 0; n < text.apiData.Count; n++)
        ...
        private class TextCaret : TexturedBox
        {
            ...
            public void Move(Vector2I dir, bool navigate = false)
            {
                ...
                {
                    var newOffset = Math.Max(caretOffset + dir.Y, 0);
            ...
            {
                var index = GetIndexFromOffset(offset);
            ...
            protected override void Draw()
            {
                if (ShowCaret)
                {
                    var isCharVisible = text.Count == 0 || text[0].Count == 0;
                        ...
                        CaretIndex.X >= text.VisibleLineRange.X && CaretIndex.X <= text.VisibleLineRange.Y)
                    {
                        ...
                        var index = Vector2I.Max(CaretIndex, Vector2I.Zero);
                        ...
                        BoundingBox2 textBounds = BoundingBox2.CreateFromHalfExtent(Vector2.Zero, .5f * text.Size),
                            charBounds =
                                BoundingBox2.CreateFromHalfExtent(pos, .5f * Vector2.Max(size, new Vector2(8f)));
            ...
            private void GetClickedChar(Vector2 cursorPos)
            {
                if ((cursorPos - lastCursorPos).LengthSquared() > 4f)
                {
                    ...
                    var offset = cursorPos - textElement.Position;
            ...
            private int GetOffsetFromIndex(Vector2I index)
            {
                ...
                for (var line = 0; line < index.X; line++) offset += text[line].Count;
            ...
            private Vector2I GetIndexFromOffset(int offset)
            {
                ...
                for (var line = 0; line < text.Count; line++)
                {
                    var lineLength = text[line].Count;
        ...
        private class SelectionBox : HudElementBase
        {
            ...
            public Color Color
            {
                get { return highlightBoard.Color; }
                set { highlightBoard.Color = value; }
            }
            ...
            {
                var caretIndex = caret.CaretIndex;
            ...
            protected override void Draw()
            {
                ...
                if (!Empty)
                {
                    ...
                    for (var n = 0; n < highlightList.Count; n++)
            ...
            private void UpdateHighlight()
            {
                ...
                // Clamp line range
                var lineRange = text.VisibleLineRange;
            ...
            private void AddHighlightBox(int line, int startCh, int endCh)
            {
                if (text[line].Count > 0)
                {
                    if (startCh < 0 || endCh < 0 || startCh >= text[line].Count || endCh >= text[line].Count)
                        throw new Exception(
                            $"Char out of range. Line: {line} StartCh: {startCh}, EndCh: {endCh}, Count: {text[line].Count}");
            ...
            private struct HighlightBox
            {
                ...
                public void Draw(MatBoard matBoard, Vector2 origin, Vector2 tbOffset, Vector2 xBounds,
                    MatrixD[] matrixRef)
                {

...
{
    /// <summary>
    ///     Handles basic file I/O operations in local storage. Will not allow multiple threads to operate on the same file
    ///     object concurrently.
    /// </summary>
    public class LocalFileIO
    {
        ...
        {
            string data;
            var exception = TryRead(out data);
            LocalFileIO newFile;
        ...
        {
            string current;
            var exception = TryRead(out current);

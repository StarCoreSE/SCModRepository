...
namespace Jitter.Collision
{
    ...
    public class Octree
    {
        ...
        internal BoundingBox rootNodeBox;

        /// <summary>
        /// Gets the root node box containing the whole octree.
        /// </summary>
        public BoundingBox RootNodeBox { get { return rootNodeBox; } }
        ...
        public void BuildOctree()
        {
            ...
            for (int triNum = 0; triNum < tris.Length; triNum++)
            {
                ...
                while (box.Contains(ref triBoxes[triNum]) == BoundingBox.ContainmentType.Contains)
                {
                    int childCon = -1;
                    for (int i = 0; i < 8; ++i)
                    {
                        CreateAABox(ref box, (EChild)i, out children[i]);
                        if (children[i].Contains(ref triBoxes[triNum]) == BoundingBox.ContainmentType.Contains)
                        {
        ...
        #region  private void CreateAABox(ref BoundingBox aabb, EChild child,out BoundingBox result)
        private void CreateAABox(ref BoundingBox aabb, EChild child, out BoundingBox result)
        {
            Vector3 dims;
        ...
        /// <returns></returns>
        #region public int GetTrianglesIntersectingtAABox(List<int> triangles, ref BoundingBox testBox)
        public int GetTrianglesIntersectingtAABox(List<int> triangles, ref BoundingBox testBox)
        {
            if (nodes.Length == 0)
                return 0;
            int curStackIndex = 0;
            ...
            while (curStackIndex < endStackIndex)
            {
                ...
                curStackIndex++;
                if (nodes[nodeIndex].box.Contains(ref testBox) != BoundingBox.ContainmentType.Disjoint)
                {
                    for (int i = 0; i < nodes[nodeIndex].triIndices.Length; ++i)
                    {
                        if (triBoxes[nodes[nodeIndex].triIndices[i]].Contains(ref testBox) != BoundingBox.ContainmentType.Disjoint)
                        {
